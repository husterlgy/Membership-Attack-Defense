...
def train_shadow_models(n_hidden=50, epochs=100, n_shadow=20, learning_rate=0.05, batch_size=100, l2_ratio=1e-7,
                        model='nn', save=True, DATA_PATH = './data/', MODEL_PATH = './model/'):
    ...
    for i in xrange(n_shadow):
        ...
        data = load_data(DATA_PATH+'shadow{}_data.npz'.format(i))
...
def save_data(dataset,args, DATA_PATH = './data/', MODEL_PATH = './model/'):
...
#    x, y, test_x, test_y = load_dataset(args.train_feat, args.train_label, args.test_feat, args.train_label)
    x, y, test_x, test_y = dataset#不从原始代码的入口导入数据，dataset是我们自己处理好保存为npz的原始和训练数据
    if test_x == None:
